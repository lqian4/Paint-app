...
io.sockets.on('connection',function (socket) {
    socket.on('login',function (name) {
        ...
        this.attrin = false;
        this.emit('server msg','Welcome, '+this.name+' !');
        this.broadcast.emit('server msg','Welcome, '+this.name+' !');
        ...
        this.on('in',function () {
            ...
            setTimeout(function () {
                ...
                Game.timer = setTimeout(function () {
                    ...
                    t.time--;var o = {name:t.name,time:t.time,word:t.word.word.length+'words'};
                    if(t.time <= 30) {
            ...
            if(!doCmd(msg,this)) {
                ...
                if(Game.player && Game.player.word.word === msg){
                    if(this.prev && this.prev.player === Game.player&& this.prev.word === msg){
                        this.emit('server msg',"");
                        return;
                    }
                    tops.set(this.id.substring(2),this.name,tops.get(this.id.substring(2)).v+1);
                    this.emit('server msg',"Correct！");
                    this.broadcast.emit('server msg',"Congratulations!!"+this.name+" Correct answer!");
                    var j = JSON.stringify(tops);
                    this.broadcast.emit('tops',j);
                    this.emit('tops',j);
                    this.prev = {
                ...
                this.emit('server msg',date+'<br>'+ this.name  + ' says: ' + msg);
                this.broadcast.emit('server msg',date+'<br>'+ this.name  + ' says: ' + msg);
            }
        });
        this.on('disconnect',function () {
            if(Game.player && this.id === Game.player.id) {
                ...
                this.broadcast.emit('othertime',JSON.stringify({name:this.name+'(quited)',time:0}));
            ...
            this.broadcast.emit('server msg','bye, '+this.name +'。');
            this.broadcast.emit('out user',this.id.substring(2));
